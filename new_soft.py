...
# Define the button number to listen for
BUTTON_TO_TRIGGER = 10  # Button 10

def run_gstreamer_command():
    """Runs the GStreamer command."""
    command = (
...
class PrototypeSoftware(QWidget):
    ...
    def check_joystick(self):
        if self.joystick:
            ...
            if buttons[6]:
                print("Grip Open")
                self.send_command('gripOpen')
            else:
                pass

            if buttons[7]:
                print("Grip Close")
                self.send_command('gripClose')
            else:
                pass
            ...
            if buttons[8]:
                ...
                self.send_command('rotateClockwise')
            else:
                pass
            if buttons[9]:
                print("Rotate Anticlockwise")
                self.send_command('rotateAnticlockwise')
            else:
                pass
