...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (isValidCommand(command)) {
      ...
      // Process the command
      if (command == "up") {
        int motors[] = {4, 1}; // Channels for upward movement
        moveMotors(motors, 2, pwmFullThrottle);
      } else if (command == "right") {
        int motors[] = {5, 0}; // Channels for right movement
        moveMotors(motors, 2, pwmFullThrottle);
      } else if (command == "left") {
        int motors[] = {3, 2}; // Channels for left movement
        moveMotors(motors, 2, pwmFullThrottle);
      } else if (command == "forward") {
        int motors[] = {0, 3}; // Channels for forward movement
        moveMotors(motors, 2, pwmFullThrottle);
      } else if (command == "backward") {
        int motors[] = {5, 2}; // Channels for backward movement
        moveMotors(motors, 2, pwmFullThrottle);
      } else if (command == "all") {
